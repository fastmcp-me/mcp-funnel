...
describe('HackyDiscovery Real Behavior', () => {
  ...
  it('dramatically reduces context size', () => {
    ...
    const hackyTools = [
      { name: 'discover_tools_by_words', description: 'Search for tools by keywords in their descriptions. Returns matching tools that can be dynamically enabled to reduce context usage.', inputSchema: { type: 'object', properties: { words: { type: 'string' }, enable: { type: 'boolean' } } } },
      { name: 'get_tool_schema', description: 'Get the input schema for a specific tool. Use the returned schema to understand what arguments are required for bridge_tool_request.', inputSchema: { type: 'object', properties: { tool: { type: 'string' } } } },
      { name: 'bridge_tool_request', description: 'Execute any discovered tool dynamically. First use get_tool_schema to understand the required arguments structure.', inputSchema: { type: 'object', properties: { tool: { type: 'string' }, arguments: { type: 'object' } } } }
    ];
